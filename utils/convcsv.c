#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <unistd.h>
#include <sys/time.h>

#include <gmp.h>

typedef unsigned long long int us_t;

us_t getus()
{
  struct timeval tv;

  us_t mult = 1000000;
  us_t s, us;
  gettimeofday(&tv, NULL);
  s = tv.tv_sec;
  us = tv.tv_usec;

  return (mult * s + us);
}

void usage()
{
  printf("USAGE: ./convcsv [csv_file] [out_file] [label_file] [minor_file]\n");
}

int getnextperm(int n, int r, int *arr, int first)
{
  if(first) {
    for(int i = 0; i < r; i++)
      arr[i] = i;

    return 0;
  }

  for(int i = 1; i < (r + 1); i++) {
    if(arr[r - i] < (n - i)) {
      arr[r - i]++;
      for(int j = (r - i + 1); j < r; j++)
        arr[j] = arr[r - i] + (j - r + i);
      return 0;
    }
  }

  return -1;
}

int main(int argc, char **argv)
{
  us_t start = getus();

  extern char *optarg;
  int max_card = 2;
  double min_support = 0.0;
  int make_minor = 0;
  char ch;
  while((ch = getopt(argc, argv, "c:s:m:")) != -1) {
    switch(ch) {
    case 'c':
      max_card = atoi(optarg);
      if(max_card < 1) {
        printf("Max cardinality must be greater than 0\n");
        return 1;
      }
      break;
    case 's':
      min_support = atof(optarg);
      if(min_support >= 1.0 || min_support <= 0.0) {
        printf("Minimum support must be less than 1 and greater than 0\n");
        return 1;
      }
      break;
    case 'm':
      make_minor = 1;
      break;
    default:
      printf("Unknown option '%c'\n", ch);
      usage();
      return 1;
    };
  }
 
  int ret = 0, nrules = 0, nfeatures = 0, nsamples = 0;
  int *rule_ids = NULL, *real_ids = NULL;
  char **features = NULL, **rules = NULL;
  char *rule_str = NULL;
  mpz_t *rules_vec = NULL;
  char *labels[2];
  labels[0] = NULL;
  labels[1] = NULL;

  FILE *csv_fp = NULL, *out_fp = NULL, *label_fp = NULL, *minor_fp = NULL;

  if((argc - optind) < 3) {
    usage();
    return 1;
  }

  csv_fp = fopen(argv[optind], "r");
  if(!csv_fp) {
    fprintf(stderr, "Could not open csv file!\n");
    ret = 1;
    goto end;
  }

  out_fp = fopen(argv[optind + 1], "w");
  if(!out_fp) {
    fprintf(stderr, "Could not open/create .out file!\n");
    ret = 1;
    goto end;
  }

  label_fp = fopen(argv[optind + 2], "w");
  if(!label_fp) {
    fprintf(stderr, "Could not open/create .label file!\n");
    ret = 1;
    goto end;
  }

  if(make_minor) {
    minor_fp = fopen(argv[4], "w");
    if(!minor_fp) {
      fprintf(stderr, "Could not open/create .minor file!\n");
      ret = 1;
      goto end;
    }
  }

  nsamples = 0;
  int c;
  
  // Get the upper bound on the number of features
  nfeatures = 1;
  while(1) {
    c = getc(csv_fp);
    if(c == '\n')
      break;
    else if(c == ',')
      nfeatures++;
  }

  features = malloc(sizeof(char*) * nfeatures);

  // Get the upper bound on the number of samples
  while(1) {
    c = getc(csv_fp);

    if(c == '\n')
      nsamples++;
    else if(c == EOF) {
      nsamples++;
      break;
    }
  }

  labels[0] = malloc(2 * nsamples + 1);
  labels[1] = malloc(2 * nsamples + 1);

  rewind(csv_fp);

  char *line;
  size_t n = 0;
  int firstline = 1, sample = 0;
  nfeatures = 0;
  while(getline(&line, &n, csv_fp) != -1) {
    if(firstline) {
      char *tok = strtok(line, ",\n");
      while(tok != NULL) {
        if(tok[0] == '\n' || tok[0] == '\0')
          continue;

        nfeatures++;
        features[nfeatures - 1] = strdup(tok);
        tok = strtok(NULL, ",\n");
      }

      if((nfeatures - 1) < max_card) {
        printf("Max cardinality must be smaller than or equal to the number of rules\n");
        ret = 1;
        goto end;
      }

      // two for each feature - normal and not
      nrules = 2 * (nfeatures - 1);
      rules = malloc(sizeof(char*) * nrules);

      for(int i = 0; i < nrules; i++)
        rules[i] = malloc(2 * nsamples + 1);

      firstline = 0;
    }
    else {
      char *tok = strtok(line, ",\n");

      // counter for the number of features read for this sample
      int i = 0;

      while(tok != NULL) {
        if(i == (nrules / 2)) {
          int num = atoi(tok);

          labels[0][2 * sample] = !num + '0';
          labels[0][2 * sample + 1] = ' ';
          labels[1][2 * sample] = !!num + '0';
          labels[1][2 * sample + 1] = ' ';
          break;
        }

        int bit = atoi(tok);
        rules[i][2 * sample] = !!bit + '0';
        rules[i][2 * sample + 1] = ' ';
        rules[nrules / 2 + i][2 * sample] = !bit + '0';
        rules[nrules / 2 + i][2 * sample + 1] = ' ';
        i++;

        tok = strtok(NULL, ",\n");
      }

      if(i != (nrules / 2))
        nsamples--;
      else
        sample++;
    }

    free(line);
    n = 0;
  }

  for(int i = 0; i < nrules; i++)
    rules[i][2 * nsamples - 1] = '\0';

  labels[0][2 * nsamples - 1] = '\0';
  labels[1][2 * nsamples - 1] = '\0';

  for(int i = 0; i < (nrules / 2); i++)
    fprintf(out_fp, "{%s} %s\n", features[i], rules[i]);

  for(int i = 0; i < (nrules / 2); i++)
    fprintf(out_fp, "{%s-not} %s\n", features[i], rules[i + nrules / 2]);

  fprintf(label_fp, "{label=0} %s\n", labels[0]);
  fprintf(label_fp, "{label=1} %s\n", labels[1]);

  free(labels[0]);
  labels[0] = NULL;
  free(labels[1]);
  labels[1] = NULL;

  if(max_card == 1)
    goto end;

  rules_vec = malloc(sizeof(mpz_t) * nrules);

  for(int i = 0; i < nrules; i++) {
    mpz_init2(rules_vec[i], nsamples);
    if(mpz_set_str(rules_vec[i], rules[i], 2) == -1) {
      for(int j = 0; j <= i; j++)
        mpz_clear(rules_vec[j]);

      printf("Could not convert rules to vectors\n");
      goto end;
    }
  }

  printf("nrules: %d\n", nrules);

  free(rules);
  rules = NULL;

  mpz_t gen_rule;
  mpz_init2(gen_rule, nsamples);

  int min_thresh = min_support * (double)nsamples;
  int max_thresh = (1.0 - min_support) * (double)nsamples;

  rule_str = malloc(nsamples + 1);

  rule_ids = malloc(sizeof(int) * max_card);
  real_ids = malloc(sizeof(int) * (nrules / 2));

  for(int card = 2; card <= max_card; card++) {
    int r = getnextperm(nrules, card, rule_ids, 1);

    while(r != -1) {
      int next_perm = 0;

      // real_ids[i] is set to 1 if it is included, else 0
      for(int i = 0; i < (nrules / 2); i++)
        real_ids[i] = 0;

      for(int i = 0; i < card; i++) {
        if(real_ids[rule_ids[i] % (nrules / 2)]) {
          next_perm = 1;
          break;
        }
        real_ids[rule_ids[i] % (nrules / 2)] = 1;
      }

      if(next_perm) {
        r = getnextperm(nrules, card, rule_ids, 0);
        continue;
      }

      mpz_set(gen_rule, rules_vec[rule_ids[0]]);

      int valid = 1;

      for(int i = 0; i < card; i++) {
        mpz_and(gen_rule, rules_vec[rule_ids[i]], gen_rule);
        int ones = mpz_popcount(gen_rule);
        if(ones <= min_thresh || ones >= max_thresh) {
          valid = 0;
          break;
        }
      }

      if(valid) {
        mpz_get_str(rule_str, 2, gen_rule);

        fprintf(out_fp, "{%s", features[rule_ids[0] % (nrules / 2)]);
        if(rule_ids[0] >= (nrules / 2))
          fprintf(out_fp, "-not");
        for(int i = 1; i < card; i++) {
          fprintf(out_fp, ",%s", features[rule_ids[i] % (nrules / 2)]);
          if(rule_ids[i] >= (nrules / 2))
            fprintf(out_fp, "-not");
        }
        fprintf(out_fp, "} ");

        int l = strlen(rule_str);

        for(int i = 0; i < (nsamples - l); i++) {
          fprintf(out_fp, "0 ");
        }

        for(int i = 0; i < l; i++) {
          fprintf(out_fp, "%c ", rule_str[i]);
        }

        fprintf(out_fp, "\n");
      }

      /*printf("Perm: ");
      for(int x = 0; x < card; x++)
        printf("%d, ", rule_ids[x]);
      printf("\n");*/

      r = getnextperm(nrules, card, rule_ids, 0);
    }
  }

  mpz_clear(gen_rule);

  for(int i = 0; i < nrules; i++)
    mpz_clear(rules_vec[i]);

end:
  if(csv_fp)
    fclose(csv_fp);
  if(out_fp)
    fclose(out_fp);
  if(label_fp)
    fclose(label_fp);
  if(minor_fp)
    fclose(minor_fp);

  if(features) {
    for(int i = 0; i < nfeatures; i++) {
      if(features[i])
        free(features[i]);
    }

    free(features);
  }

  if(rules) {
    for(int i = 0; i < nrules; i++)
      if(rules[i])
        free(rules[i]);

    free(rules);
  }

  if(labels[0])
    free(labels[0]);

  if(labels[1])
    free(labels[1]);

  if(rules_vec)
    free(rules_vec);

  if(rule_str)
    free(rule_str);

  if(rule_ids)
    free(rule_ids);

  if(real_ids)
    free(real_ids);

  long double diff = (long double)(getus() - start) / (long double)1000000.0;
  printf("Done after %Lf seconds\n", diff);

  return ret;
}
